...
# Visualisasi data dengan grouped bar chart
chart = (
    alt.Chart(day_df)
    .mark_bar()
    .encode(x="yr:O", y="cnt:Q", color="yr:N", column="season:N")
)

st.write(chart)
st.markdown(
    '''
Dapat diketahui berdasarkan peminjaman sepeda pada tahun 2011, penyewaan sepeda terbanyak berada di musim gugur, 
sedangkan penyewaan sepeda paling sedikit terdapat pada musim semi.
'''
)
...
st.markdown(
    '''
Berdasarkan line chart dapat dilihat adanya pola atau siklus yang berulang dalam 1 tahun di tahun berikutnya. 
Hal ini dapat dilihat pada bulan Januari, line chart berada di titik rendah kemudian perlahan naik hingga bulan Agustus dan kembali menurun ketika menuju Desember. 
Pola ini dapat dikatakan pola musiman yang diiringi oleh tren naik.
'''
)
